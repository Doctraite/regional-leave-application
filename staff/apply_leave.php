...
/* 	if($fromdate > $todate)
	{
	    echo "<script>alert(' Date of come back should be greater than Start Date');</script>";
	  } */ 
	if($leave_days <= 0)
	{
		...
		$diff =  date_diff( $DT, $DF);
		$num_days = (1 + $diff->format("%a"));

		$sql="INSERT INTO tblleaves(LeaveType,ToDate,FromDate,Description,admin_status,IsRead,empid,num_days,PostingDate) VALUES(:leave_type,:fromdate,:todate,:description,:admin_status,:isread,:empid,:num_days,:datePosting)";
		$query = $dbh->prepare($sql);
		$query->bindParam(':leave_type',$leave_type,PDO::PARAM_STR);
		$query->bindParam(':fromdate',$fromdate,PDO::PARAM_STR);
		$query->bindParam(':todate',$todate,PDO::PARAM_STR);
		$query->bindParam(':description',$description,PDO::PARAM_STR);
		$query->bindParam(':admin_status',$status,PDO::PARAM_STR);
						...
						<form method="post" action="">
							<section>
								...
								<div class="row">
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											<label>Available Vacational Leave Days</label>
											<input name="email" type="text" class="form-control" required="true" autocomplete="off" readonly value="<?php echo $row['V_leave_days']; ?>">
										</div>
									</div>
									<div class="col-md-6 col-sm-12">
										<div class="form-group">
											<label>Available Casual Leave Days </label>
											<input name="leave_days" type="text" class="form-control" required="true" autocomplete="off" readonly value="<?php echo $row['C_leave_days']; ?>">
										</div>
									</div>
									<?php endif ?>
								</div>								
